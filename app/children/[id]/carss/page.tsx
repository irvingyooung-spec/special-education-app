...
import { CARS_DOMAINS, isPathologyDomain } from "@/lib/cars-catalog";
...
export default async function CarsHistoryPage({ params }: Props) {
  ...
  const sessions = getSessionsForChild(childId).filter(
  ...
  return (
    <PageShell
      ...
      backLabel="返回 CARS 评估"
      title={`${child.name} - CARS 评估历史`}
        ...
        {sessions.length === 0 ? (
          ...
          sessions.map((session) => {
            ...
            return (
              <Link
                key={session.id}
                href={`/children/${childId}/carss/${session.id}`}
