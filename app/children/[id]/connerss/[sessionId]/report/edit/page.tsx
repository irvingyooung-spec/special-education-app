...
import {
  getConnersReportForSession,
  CONNERS_REPORT_FIELDS,
  saveConnersReportEdits,
} from "@/lib/report";
...
export default async function EditConnersReportPage({ params }: Props) {
  ...
  const session = db
    .prepare("SELECT id FROM conners_sessions WHERE id = ? AND child_id = ?")
    .get(sessionId, childId) as { id: number } | undefined;
  if (!session) notFound();

  const report = getConnersReportForSession(sessionId);
  ...
  async function saveEdits(formData: FormData) {
    ...
    saveConnersReportEdits(sessionId, {
      factor_analysis: text("factor_analysis"),
      interpretation: text("interpretation"),
      intervention_suggestions: text("intervention_suggestions"),
      family_advice: text("family_advice"),
    });

    redirect(`/children/${childId}/connerss/${sessionId}`);
  }

  return (
    <PageShell
      backHref={`/children/${childId}/connerss/${sessionId}`}
      backLabel="返回评估详情"
    ...
    >
      <form action={saveEdits} className="space-y-5">
        ...
        <div className="flex items-center justify-between gap-4 sticky bottom-4 bg-white rounded-xl border border-[#e8e8e0] p-4 shadow-lg">
          <Link
            href={`/children/${childId}/connerss/${sessionId}`}
