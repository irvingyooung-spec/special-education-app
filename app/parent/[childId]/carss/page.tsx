...
export default async function ParentCarsHistoryPage({ params }: Props) {
  ...
  const sessions = getSessionsForChild(childIdNum).filter(
        ...
        {sessions.length === 0 ? (
          ...
          sessions.map((session) => {
            ...
            return (
              <Link
                key={session.id}
                href={`/parent/${childId}/carss/${session.id}`}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-2 space-y-1">
                      {summary.slice(0, 3).map((s) => (
                        ...
                        >
                          <span className="w-20 text-[#6b7280] truncate">
                            {s.label}
                          </span>
